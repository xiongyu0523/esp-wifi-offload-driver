...
#define AP_MODE_PROVISIONING                    1

#if (AP_MODE_PROVISIONING == 1)
...
int sample_network_configure(const CHAR *ssid_ptr, const CHAR *pwd_ptr, NX_IP *ip_ptr, ULONG *dns_address)
{
    ...
    if (WIFI_Init() == WIFI_STATUS_OK) {
        ...
        if (WIFI_IsConnected() != 1) {
            ...
            printf("Start ESP-AT Built-in Web Server for Wi-Fi provisioning\r\n");
            printf("Connect to AP and access 192.168.4.1 use browser\r\n");
            if (WIFI_ConfigureAP("ESP32 Access Point", "88888888", WIFI_ECN_WPA2_PSK, WIFI_CHANNEL, 1) == WIFI_STATUS_OK) {
                WIFI_WaitForProvision();
                WIFI_StopAP();
            } else {
        ...
        while ((retry_connect++) < RETRY_TIMES) {
            ...
            printf("Manually WIFI connect try %d times\r\n", retry_connect);
